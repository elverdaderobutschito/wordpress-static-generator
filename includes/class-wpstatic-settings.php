...
class WPStatic_Settings {
    ...
    public function enqueueAssets(string $hook): void {
        ...
        wp_localize_script('wpstatic-deploy-admin', 'wpStaticDeploy', [
            ...
            'batchSize' => 5,
            'i18n' => [
                'testConnection' => __('Test connection', 'wp-static-deploy'),
                'deployingToTarget' => __('Deploying to target …', 'wp-static-deploy'),
                'deployAll' => __('Deploy all', 'wp-static-deploy'),
                'running' => __('Running …', 'wp-static-deploy'),
                'deployingAssets' => __('Deploying assets …', 'wp-static-deploy'),
                'deployAssetsOnly' => __('Deploy assets only', 'wp-static-deploy'),
                'unsavedChangesConfirm' => __('You have unsaved changes in the settings.', 'wp-static-deploy') . "\n\n"
                    . __('"%s" uses the last SAVED settings, not your current input.', 'wp-static-deploy') . "\n\n"
                    . __('Continue anyway?', 'wp-static-deploy'),
                'unknownError' => __('Unknown error', 'wp-static-deploy'),
                'unknownErrorPeriod' => __('Unknown error.', 'wp-static-deploy'),
                'errorPrefix' => __('Error:', 'wp-static-deploy'),
                'errorRequestFailed' => __('Error processing the request.', 'wp-static-deploy'),
                'starting' => __('Starting …', 'wp-static-deploy'),
                'noMatchingPosts' => __('No matching posts/pages found.', 'wp-static-deploy'),
                'errorStartingDeploy' => __('Error starting the deployment.', 'wp-static-deploy'),
                'errorBatchProcessing' => __('Error during batch processing.', 'wp-static-deploy'),
                'errorFinalizing' => __('Error during final upload.', 'wp-static-deploy'),
                'assetsDeployedWithUrl' => __('Assets deployed. Deploy:', 'wp-static-deploy'),
                'assetsDeployed' => __('Assets deployed.', 'wp-static-deploy'),
                'deploymentComplete' => __('Deployment complete.', 'wp-static-deploy'),
                'testing' => __('Testing …', 'wp-static-deploy'),
                'requestToWordPressFailed' => __('Request to WordPress failed.', 'wp-static-deploy'),
            ],
        ]);
    }
    ...
    public function handleSave(): void {
        if (!current_user_can('manage_options')) {
            wp_die(esc_html__('Insufficient permissions.', 'wp-static-deploy'));
        }
        ...
        $newValues = [
            'target' => in_array($_POST['target'] ?? '', ['sftp', 'netlify'], true) ? $_POST['target'] : 'sftp',
            ...
            'sftp_auth_method' => in_array($_POST['sftp_auth_method'] ?? '', ['password', 'key'], true) ? $_POST['sftp_auth_method'] : 'password',
            ...
            'form_honeypot_field' => preg_replace('/[^a-zA-Z0-9_-]/', '', $_POST['form_honeypot_field'] ?? '') ?: '_gotcha',
            ...
            'nav_main_menu_id' => (int) ($_POST['nav_main_menu_id'] ?? 0),
            ...
            'nav_footer_menu_id' => (int) ($_POST['nav_footer_menu_id'] ?? 0),
            ...
            'nav_active_class' => preg_replace('/[^a-zA-Z0-9_ -]/', '', $_POST['nav_active_class'] ?? '') ?: 'active',
        ];
        ...
        if (!empty($_FILES['template_file']['tmp_name'])) {
            ...
            if (isset($uploaded['file'])) {
                ...
                set_transient('wpstatic_template_upload_error', $uploaded['error'] ?? __('Unknown error while uploading the default template.', 'wp-static-deploy'), MINUTE_IN_SECONDS * 5);
            }
        ...
        if (!empty($_POST['new_template_name'])) {
            if (!empty($_FILES['new_template_file']['tmp_name'])) {
                ...
                if (isset($uploadedTemplate['file'])) {
                    ...
                    set_transient('wpstatic_template_upload_error', $uploadedTemplate['error'] ?? __('Unknown error while uploading.', 'wp-static-deploy'), MINUTE_IN_SECONDS * 5);
                }
                ...
                set_transient('wpstatic_template_upload_error', __('A name for the new template was given, but no file was selected.', 'wp-static-deploy'), MINUTE_IN_SECONDS * 5);
            }
            ...
            set_transient('wpstatic_template_upload_error', __('Please enter a name for the new template (required) - the file was therefore not saved.', 'wp-static-deploy'), MINUTE_IN_SECONDS * 5);
        }
        ...
        foreach (self::ENCRYPTED_FIELDS as $field) {
            ...
            if ($raw !== '') {
                $newValues[$field] = WPStatic_Crypto::encrypt(wp_unslash($raw));
            } else {
        ...
        if ($newValues['forms_enabled'] === 'on' && $newValues['target'] === 'sftp' && $newValues['form_recipient_email'] === '') {
            set_transient(
                ...
                __('Forms are enabled, but no recipient email address has been set - the form handler will reject incoming submissions until this is fixed.', 'wp-static-deploy'),
                MINUTE_IN_SECONDS * 5
            );
        }
        ...
        if (isset($_FILES['assets_zip']) && $_FILES['assets_zip']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            set_transient('wpstatic_assets_upload_result', [
                ...
                'message' => sprintf(
                    ...
                    __('The request exceeds the server limit post_max_size (currently: %s). The file is too large to upload in this form.', 'wp-static-deploy'),
    ...
    public function handleMarkdownExport(): void {
        ...
        try {
            ...
            wp_die(esc_html__('Export failed:', 'wp-static-deploy') . ' ' . esc_html($e->getMessage()));
        }

        if (!is_file($zipPath)) {
            wp_die(esc_html__('Export failed: the ZIP file was not created.', 'wp-static-deploy'));
        }
    ...
    private function renderNavigationFields(string $prefix, string $label, array $settings): void {
        ...
        <table class="form-table wpstatic-nav-fields" data-prefix="<?php echo esc_attr($prefix); ?>">
            <tr>
                <th><?php esc_html_e('Generate automatically', 'wp-static-deploy'); ?></th>
                <td>
                    <label>
                        ...
                        <?php esc_html_e('Replace navigation markers in the template with real menu items', 'wp-static-deploy'); ?>
                    </label>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>menu_id"><?php esc_html_e('WordPress menu', 'wp-static-deploy'); ?></label></th>
                <td>
                    <select id="<?php echo esc_attr($prefix); ?>menu_id" name="<?php echo esc_attr($prefix); ?>menu_id">
                        <option value="0">– <?php esc_html_e('Select a menu', 'wp-static-deploy'); ?> –</option>
                    ...
                    <?php if (empty($menus)): ?>
                        <p class="description"><?php esc_html_e('No menus found - create one first under Appearance → Menus.', 'wp-static-deploy'); ?></p>
                    <?php endif; ?>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>wrapper_marker"><?php esc_html_e('Wrapper marker', 'wp-static-deploy'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Wraps the ENTIRE menu area in the template (e.g. the outer <ul>).', 'wp-static-deploy'); ?></p>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>item_marker"><?php esc_html_e('Item marker', 'wp-static-deploy'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Wraps EXACTLY ONE demo menu item without sub-items (e.g. one <li>) - duplicated for each real menu item.', 'wp-static-deploy'); ?></p>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>parent_item_marker"><?php esc_html_e('Parent item marker (optional)', 'wp-static-deploy'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Only needed for multi-level menus. Without this marker, sub-items are ignored (flat display).', 'wp-static-deploy'); ?></p>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>submenu_wrapper_marker"><?php esc_html_e('Submenu wrapper marker', 'wp-static-deploy'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Only relevant if "Parent item marker" is set - wraps the container for sub-items INSIDE the parent template.', 'wp-static-deploy'); ?></p>
                </td>
            </tr>
            <tr class="wpstatic-nav-detail">
                <th><label for="<?php echo esc_attr($prefix); ?>submenu_item_marker"><?php esc_html_e('Submenu item marker (optional)', 'wp-static-deploy'); ?></label></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Leave empty to use the same template as "Item marker" for sub-items.', 'wp-static-deploy'); ?></p>
                </td>
            </tr>
        </table>
        <?php
        ...
        switch ($error) {
            case UPLOAD_ERR_INI_SIZE:
                return sprintf(
                    ...
                    __('The file exceeds the server limit upload_max_filesize (currently: %s). Please shrink the ZIP or ask your host to raise the limit.', 'wp-static-deploy'),
                    ini_get('upload_max_filesize')
                );
            case UPLOAD_ERR_FORM_SIZE:
                return __('The file exceeds the maximum allowed by the form.', 'wp-static-deploy');
            case UPLOAD_ERR_PARTIAL:
                return __('The file was only partially uploaded. Please try again.', 'wp-static-deploy');
            case UPLOAD_ERR_NO_TMP_DIR:
                return __('Server error: no temporary directory available for uploads.', 'wp-static-deploy');
            case UPLOAD_ERR_CANT_WRITE:
                return __('Server error: the file could not be written to disk.', 'wp-static-deploy');
            case UPLOAD_ERR_EXTENSION:
                return __('The upload was stopped by a PHP extension.', 'wp-static-deploy');
            default:
                return sprintf(
                    /* translators: %d: PHP upload error code */
                    __('Unknown upload error (code %d).', 'wp-static-deploy'),
                    $error
                );
        }
    }
    ...
    public function renderPage(): void {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Static Deploy – Settings', 'wp-static-deploy'); ?></h1>

            <div id="wpstatic-unsaved-notice" class="notice notice-warning" style="display:none;">
                <p>
                    <strong><?php esc_html_e('You have unsaved changes.', 'wp-static-deploy'); ?></strong>
                    <?php esc_html_e('"Deploy all" and "Deploy assets only" use the last', 'wp-static-deploy'); ?>
                    <strong><?php esc_html_e('saved', 'wp-static-deploy'); ?></strong> <?php esc_html_e('settings, not your current input - please click "Save settings" first.', 'wp-static-deploy'); ?>
                </p>
            </div>

            <?php if (isset($_GET['saved'])): ?>
                <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Settings saved.', 'wp-static-deploy'); ?></p></div>
            <?php endif; ?>
            ...
            if ($assetsUploadResult) {
                ...
                <div class="notice <?php echo esc_attr($noticeClass); ?> is-dismissible">
                    ...
                    <?php if (!empty($assetsUploadResult['warnings'])): ?>
                        <p><strong><?php esc_html_e('Warning: suspicious file types found in the assets', 'wp-static-deploy'); ?></strong> (<?php esc_html_e('included anyway, please review', 'wp-static-deploy'); ?>):</p>
            ...
            <?php if (!WPStatic_Crypto::usesConfigKey()): ?>
                <div class="notice notice-warning">
                    <p>
                        <?php esc_html_e('No', 'wp-static-deploy'); ?> <code>WPSTATIC_ENCRYPTION_KEY</code> <?php esc_html_e('found in', 'wp-static-deploy'); ?> <code>wp-config.php</code>.
                        <?php esc_html_e('Secrets are still stored encrypted, but with an auto-generated key that also lives in the database. Recommended:', 'wp-static-deploy'); ?>
                        <code>define('WPSTATIC_ENCRYPTION_KEY', '<?php esc_html_e('a-long-random-value', 'wp-static-deploy'); ?>');</code>
                        <?php esc_html_e('add to wp-config.php.', 'wp-static-deploy'); ?>
                    </p>
                </div>
            <?php endif; ?>
            ...
            <?php if ($settings['target'] === 'sftp' && !$hasVendor): ?>
                <div class="notice notice-error">
                    <p>
                        <?php esc_html_e('SFTP target selected, but', 'wp-static-deploy'); ?> <code>vendor/phpseclib</code> <?php esc_html_e('was not found. See README.md in the plugin directory for installation.', 'wp-static-deploy'); ?>
                    </p>
                </div>
            <?php endif; ?>
            ...
            <form id="wpstatic-settings-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
                ...
                <h2 class="nav-tab-wrapper wpstatic-tabs">
                    <a href="#" class="nav-tab" data-tab="inhalte"><?php esc_html_e('Content', 'wp-static-deploy'); ?></a>
                    <a href="#" class="nav-tab" data-tab="navigation"><?php esc_html_e('Navigation', 'wp-static-deploy'); ?></a>
                    <a href="#" class="nav-tab" data-tab="formulare"><?php esc_html_e('Forms', 'wp-static-deploy'); ?></a>
                    <a href="#" class="nav-tab" data-tab="ziel"><?php esc_html_e('Deployment target', 'wp-static-deploy'); ?></a>
                    <a href="#" class="nav-tab" data-tab="markdown"><?php esc_html_e('Markdown export', 'wp-static-deploy'); ?></a>
                </h2>

                <div class="wpstatic-tab-panel" data-tab-panel="inhalte">
                <h2 class="title"><?php esc_html_e('Content', 'wp-static-deploy'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Post Types', 'wp-static-deploy'); ?></th>
                        <td>
                            <label><input type="checkbox" name="post_types[]" value="post" <?php checked(in_array('post', $settings['post_types'], true)); ?>> <?php esc_html_e('Posts', 'wp-static-deploy'); ?></label><br>
                            <label><input type="checkbox" name="post_types[]" value="page" <?php checked(in_array('page', $settings['post_types'], true)); ?>> <?php esc_html_e('Pages', 'wp-static-deploy'); ?></label>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="wpstatic_template"><?php esc_html_e('Template file', 'wp-static-deploy'); ?></label></th>
                        <td>
                            <?php if (!empty($settings['template_path'])): ?>
                                <p><?php esc_html_e('Current:', 'wp-static-deploy'); ?> <code><?php echo esc_html(basename($settings['template_path'])); ?></code></p>
                            ...
                            <p class="description"><?php esc_html_e('HTML file with placeholders (e.g.', 'wp-static-deploy'); ?> <code>###title###</code>). <?php esc_html_e('Leave empty to keep the current file.', 'wp-static-deploy'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Additional templates', 'wp-static-deploy'); ?></th>
                        <td>
                            <?php if (!empty($settings['extra_templates'])): ?>
                                <table class="widefat" style="max-width: 500px; margin-bottom: 10px;">
                                    <thead><tr><th><?php esc_html_e('Name', 'wp-static-deploy'); ?></th><th><?php esc_html_e('File', 'wp-static-deploy'); ?></th><th><?php esc_html_e('Remove', 'wp-static-deploy'); ?></th></tr></thead>
                                    <tbody>
                            ...
                            <?php else: ?>
                                <p class="description"><?php esc_html_e('No additional templates set up yet.', 'wp-static-deploy'); ?></p>
                            <?php endif; ?>

                            <p>
                                <label for="new_template_name"><?php esc_html_e('Add new template (name required):', 'wp-static-deploy'); ?></label><br>
                                <input type="text" id="new_template_name" name="new_template_name" class="regular-text" placeholder="<?php echo esc_attr__('e.g. Landing page (required so it appears in the dropdown)', 'wp-static-deploy'); ?>">
                                <input type="file" id="new_template_file" name="new_template_file" accept=".html,.htm">
                            </p>
                            <p class="description">
                                <?php esc_html_e('Then available per post/page as an alternative to the default template (dropdown in the "Static Deploy" meta box on the edit screen).', 'wp-static-deploy'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($assetsStatus['exists']): ?>
                                <p>
                                    <?php esc_html_e('Currently set up:', 'wp-static-deploy'); ?> <strong><?php echo esc_html((string) $assetsStatus['file_count']); ?> <?php esc_html_e('files', 'wp-static-deploy'); ?></strong>
                                    (<?php echo esc_html(size_format($assetsStatus['total_size'])); ?>)
                                    <?php if (!empty($assetsStatus['updated_at'])): ?>
                                        &ndash; <?php esc_html_e('last updated on', 'wp-static-deploy'); ?> <?php echo esc_html(date_i18n('d.m.Y H:i', $assetsStatus['updated_at'])); ?>
                                    <?php endif; ?>
                                </p>
                            <?php else: ?>
                                <p><?php esc_html_e('No assets set up yet.', 'wp-static-deploy'); ?></p>
                            <?php endif; ?>
                            <input type="file" id="wpstatic_assets" name="assets_zip" accept=".zip">
                            <p class="description">
                                <?php esc_html_e('ZIP file whose root already is a folder named', 'wp-static-deploy'); ?> <code>assets/</code>
                                (<?php esc_html_e('i.e. zip the assets folder itself, not just its contents', 'wp-static-deploy'); ?>).
                                <?php esc_html_e('Copied 1:1 into the build folder so template references like', 'wp-static-deploy'); ?>
                                <code>href="assets/css/styles.css"</code> <?php esc_html_e('work.', 'wp-static-deploy'); ?>
                            </p>
                            <p class="description">
                                <?php esc_html_e('Server upload limit: max.', 'wp-static-deploy'); ?> <code><?php echo esc_html(ini_get('upload_max_filesize')); ?></code>
                                <?php esc_html_e('per file,', 'wp-static-deploy'); ?> <code><?php echo esc_html(ini_get('post_max_size')); ?></code> <?php esc_html_e('per form submission overall. If your ZIP is bigger, please compress/trim assets first (e.g. remove unused font weights) or ask your host to raise the limit.', 'wp-static-deploy'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="date_format"><?php esc_html_e('Date format', 'wp-static-deploy'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="remove_wp_tags"><?php esc_html_e('Remove WP CSS classes', 'wp-static-deploy'); ?></label></th>
                        <td><label><input type="checkbox" id="remove_wp_tags" name="remove_wp_tags" <?php checked('on', $settings['remove_wp_tags']); ?>> <?php esc_html_e('active', 'wp-static-deploy'); ?></label></td>
                    </tr>
                    <tr>
                        <th><label for="data_injection_rules"><?php esc_html_e('Data injection rules', 'wp-static-deploy'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="change_url_rules"><?php esc_html_e('Change-URL rules', 'wp-static-deploy'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="tidy_html_rules"><?php esc_html_e('Tidy HTML rules', 'wp-static-deploy'); ?></label></th>
                ...
                <h2 class="title"><?php esc_html_e('Forms', 'wp-static-deploy'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Enable forms', 'wp-static-deploy'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="forms_enabled" <?php checked('on', $settings['forms_enabled']); ?>>
                                <?php esc_html_e('Make &lt;form&gt; tags on generated pages functional', 'wp-static-deploy'); ?>
                            </label>
                            <p class="description">
                                <strong>Netlify:</strong> <?php esc_html_e('forms automatically get', 'wp-static-deploy'); ?> <code>data-netlify="true"</code>
                                <?php esc_html_e('and a hidden', 'wp-static-deploy'); ?> <code>form-name</code> <?php esc_html_e('field - Netlify Forms then detects and processes them on its own, no server code needed.', 'wp-static-deploy'); ?><br>
                                <strong>SFTP:</strong> <?php esc_html_e('the form', 'wp-static-deploy'); ?> <code>action</code> <?php esc_html_e('is rewritten to a bundled PHP handler', 'wp-static-deploy'); ?> (<code>form-handler.php</code>) <?php esc_html_e('that emails submissions. Requires the target server to run PHP.', 'wp-static-deploy'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_recipient_email"><?php esc_html_e('Recipient email (SFTP only)', 'wp-static-deploy'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="form_from_email"><?php esc_html_e('Sender address (SFTP only)', 'wp-static-deploy'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Leave empty to use the server default address. Many hosts only accept sender addresses from their own domain.', 'wp-static-deploy'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_subject_prefix"><?php esc_html_e('Subject prefix (SFTP only)', 'wp-static-deploy'); ?></label></th>
                    ...
                    <tr>
                        <th><label for="form_redirect_url"><?php esc_html_e('Thank-you page (Netlify + SFTP)', 'wp-static-deploy'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('Leave empty to redirect back to the originating page after submission automatically. Important for Netlify: the form\'s original', 'wp-static-deploy'); ?> <code>action</code> (<?php esc_html_e('points to a WordPress URL', 'wp-static-deploy'); ?>) <?php esc_html_e('is always replaced - either by this thank-you page, or, if empty, by removing the', 'wp-static-deploy'); ?> <code>action</code> (<?php esc_html_e('submits to the current page', 'wp-static-deploy'); ?>).
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_honeypot_field"><?php esc_html_e('Honeypot field name (SFTP only)', 'wp-static-deploy'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Simple spam deterrent: a CSS-hidden field that only bots fill in. Letters/numbers/underscore/hyphen only.', 'wp-static-deploy'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="form_custom_action"><?php esc_html_e('Custom form target (SFTP only)', 'wp-static-deploy'); ?></label></th>
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('Leave empty to use our bundled PHP handler', 'wp-static-deploy'); ?> (<code>/form-handler.php</code>) - <?php esc_html_e('requires the target server to run PHP. If something else runs there (e.g. your own endpoint, an external form service), enter its URL here - the form', 'wp-static-deploy'); ?> <code>action</code> <?php esc_html_e('is then rewritten to it, and our', 'wp-static-deploy'); ?> <code>form-handler.php</code> <?php esc_html_e('is not created/uploaded in that case.', 'wp-static-deploy'); ?>
                            </p>
                        </td>
                    </tr>
                </table>

                <h3><?php esc_html_e('Client-side validation', 'wp-static-deploy'); ?></h3>
                <p class="description">
                    <?php esc_html_e('Runs automatically as soon as forms are enabled above (for Netlify AND SFTP) - no extra toggle needed. Checks in the browser BEFORE submitting: fields with', 'wp-static-deploy'); ?>
                    <code>aria-required="true"</code> <?php esc_html_e('must be filled in (for checkbox/radio groups: at least one option),', 'wp-static-deploy'); ?>
                    <code>type="email"</code> <?php esc_html_e('fields must look like an email address. Most accessible form plugins (Fluent Forms, WPForms, Gravity Forms etc.) already set these attributes automatically - no configuration needed. If your form plugin doesn\'t set these attributes, the check simply doesn\'t apply (data is then submitted unvalidated, as before).', 'wp-static-deploy'); ?>
                </p>
                </div>

                <div class="wpstatic-tab-panel" data-tab-panel="navigation">
                <h2 class="title"><?php esc_html_e('Navigation', 'wp-static-deploy'); ?></h2>
                <p class="description">
                    <?php esc_html_e('Replaces marker comments in the template with real menu items from a WordPress menu. Details for each marker are right next to the respective fields below.', 'wp-static-deploy'); ?>
                </p>
                <?php $this->renderNavigationFields('nav_main_', __('Main navigation', 'wp-static-deploy'), $settings); ?>
                <?php $this->renderNavigationFields('nav_footer_', __('Footer navigation', 'wp-static-deploy'), $settings); ?>
                <table class="form-table">
                    <tr>
                        <th><label for="nav_active_class"><?php esc_html_e('CSS class for active page', 'wp-static-deploy'); ?></label></th>
                        <td>
                            ...
                            <p class="description"><?php esc_html_e('Set on the first &lt;a&gt; tag of the menu item matching the page currently being generated - applies to main and footer navigation alike.', 'wp-static-deploy'); ?></p>
                        </td>
                    </tr>
                </table>
                </div>
                ...
                <h2 class="title"><?php esc_html_e('Deployment target', 'wp-static-deploy'); ?></h2>
                <table class="form-table">
                    <tr>
                        <th><?php esc_html_e('Target', 'wp-static-deploy'); ?></th>
                        <td>
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="description"><?php echo !empty($settings['netlify_token']) ? esc_html__('A token is already saved. Leave empty to keep it.', 'wp-static-deploy') : esc_html__('No token saved yet.', 'wp-static-deploy'); ?></p>
                            </td>
                        </tr>
                    </table>
                    <p>
                        <button type="button" class="button" id="wpstatic-test-netlify-btn"><?php esc_html_e('Test connection', 'wp-static-deploy'); ?></button>
                        <span id="wpstatic-test-netlify-result" class="wpstatic-test-result"></span>
                    </p>
                    <p class="description">
                        <?php esc_html_e('Important: Netlify deploys always replace the entire site content. The single-page button (on every post/page edit screen) therefore triggers a full rebuild + redeploy on Netlify, not just an update of that one page.', 'wp-static-deploy'); ?>
                    </p>
                </div>
                ...
                <div id="wpstatic-target-sftp" class="wpstatic-target-section">
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            <th><label for="sftp_username"><?php esc_html_e('Username', 'wp-static-deploy'); ?></label></th>
                        ...
                        <tr>
                            <th><?php esc_html_e('Authentication', 'wp-static-deploy'); ?></th>
                            <td>
                                <label><input type="radio" name="sftp_auth_method" value="password" <?php checked('password', $settings['sftp_auth_method']); ?> class="wpstatic-sftp-auth-radio"> <?php esc_html_e('Password', 'wp-static-deploy'); ?></label>
                                &nbsp;&nbsp;
                                <label><input type="radio" name="sftp_auth_method" value="key" <?php checked('key', $settings['sftp_auth_method']); ?> class="wpstatic-sftp-auth-radio"> <?php esc_html_e('Private key', 'wp-static-deploy'); ?></label>
                            </td>
                        </tr>
                        <tr class="wpstatic-sftp-auth-password">
                            <th><label for="sftp_password"><?php esc_html_e('Password', 'wp-static-deploy'); ?></label></th>
                            <td>
                                <input type="password" id="sftp_password" name="sftp_password" value="" class="regular-text" autocomplete="new-password">
                                <p class="description"><?php echo !empty($settings['sftp_password']) ? esc_html__('Already saved. Leave empty to keep it.', 'wp-static-deploy') : ''; ?></p>
                            </td>
                        </tr>
                        <tr class="wpstatic-sftp-auth-key">
                            <th><label for="sftp_private_key"><?php esc_html_e('Private key (PEM)', 'wp-static-deploy'); ?></label></th>
                            <td>
                                ...
                                <p class="description"><?php echo !empty($settings['sftp_private_key']) ? esc_html__('Already saved. Leave empty to keep it.', 'wp-static-deploy') : ''; ?></p>
                            </td>
                        </tr>
                        <tr class="wpstatic-sftp-auth-key">
                            <th><label for="sftp_passphrase"><?php esc_html_e('Passphrase (if any)', 'wp-static-deploy'); ?></label></th>
                            <td><input type="password" id="sftp_passphrase" name="sftp_passphrase" value="" class="regular-text" autocomplete="new-password"></td>
                        </tr>
                        <tr>
                            <th><label for="sftp_remote_base_path"><?php esc_html_e('Target directory on the server', 'wp-static-deploy'); ?></label></th>
                    ...
                    <p>
                        <button type="button" class="button" id="wpstatic-test-sftp-btn"><?php esc_html_e('Test connection', 'wp-static-deploy'); ?></button>
                ...
                <?php submit_button(__('Save settings', 'wp-static-deploy')); ?>
                </div>
            </form>
            ...
            <h2 class="title"><?php esc_html_e('Deployment', 'wp-static-deploy'); ?></h2>
            <p>
                <button type="button" class="button button-primary button-hero" id="wpstatic-deploy-all-btn">
                    <?php esc_html_e('Deploy all', 'wp-static-deploy'); ?>
                </button>
                <button type="button" class="button" id="wpstatic-deploy-assets-only-btn" <?php echo $assetsStatus['exists'] ? '' : 'disabled'; ?>>
                    <?php esc_html_e('Deploy assets only', 'wp-static-deploy'); ?>
                </button>
            </p>
            <p>
                <label>
                    <input type="checkbox" id="wpstatic-skip-assets" checked>
                    <?php esc_html_e('Upload assets when deploying', 'wp-static-deploy'); ?>
                </label>
                <?php if ($settings['target'] === 'netlify'): ?>
                    <br><span class="description"><?php esc_html_e('On Netlify, assets are always uploaded (a deploy replaces the entire content) - this option has no effect here.', 'wp-static-deploy'); ?></span>
                <?php endif; ?>
            </p>
            ...
            <h2 class="title"><?php esc_html_e('Markdown export', 'wp-static-deploy'); ?></h2>
            <p class="description">
                <?php esc_html_e('Exports all posts/pages as plain', 'wp-static-deploy'); ?> <code>.md</code> <?php esc_html_e('files with YAML front matter (title, date, slug) - meant for other systems (Hugo, Jekyll, Eleventy, Obsidian vault etc.),', 'wp-static-deploy'); ?>
                <strong><?php esc_html_e('not', 'wp-static-deploy'); ?></strong> <?php esc_html_e('as a deployable website. Runs independently of SFTP/Netlify; the result is a direct ZIP download.', 'wp-static-deploy'); ?>
            </p>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <p>
                    <label>
                        ...
                        <?php esc_html_e('Download images and rewrite references as relative paths', 'wp-static-deploy'); ?>
                    </label>
                    <br>
                    <span class="description">
                        <?php esc_html_e('Enabled: images are additionally included in the ZIP (under', 'wp-static-deploy'); ?> <code>wp-content/uploads/...</code>), <?php esc_html_e('references become relative - the export then also works without a running WordPress instance. Disabled: image links stay unchanged and keep pointing to your WordPress site.', 'wp-static-deploy'); ?>
                    </span>
                </p>
                <?php submit_button(__('Export as Markdown now', 'wp-static-deploy'), 'secondary'); ?>
            </form>
            </div>
        </div>
        <?php
